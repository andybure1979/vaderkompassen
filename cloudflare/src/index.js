...
async function authenticatedUser(req,env){
  const token=req.headers.get("authorization")?.replace(/^Bearer\s+/i,"").trim();
  if(!token)throw Object.assign(new Error("Inloggning krävs"),{status:401});
  const base=String(env.SUPABASE_URL||"").trim().replace(/\/+$/,'');
  const authResponse=await fetch(`${base}/auth/v1/user`,{headers:{apikey:String(env.SUPABASE_SERVICE_ROLE_KEY||""),authorization:`Bearer ${token}`}});
  if(!authResponse.ok)throw Object.assign(new Error("Ogiltig eller utgången session"),{status:401});
  return {user:await authResponse.json(),token};
}
...
const rankingStore={
  ...
  async read(env,normalized,performanceMetrics){
    ...
    return {ok:true,version:env.APP_VERSION||'15.0.6',workerVersion:env.APP_VERSION||'15.0.6',snapshotVersion:head.snapshot_version,generatedAt:head.generated_at,
...
async function buildSnapshot(env){
  ...
  return {ok:true,version:env.APP_VERSION||'15.0.6',workerVersion:env.APP_VERSION||'15.0.6',snapshotVersion:snapshotVersionFor(generatedAt),generatedAt,activeDate:days[0]||null,dailyResults,
...
async function latestSnapshot(env,normalized,performanceMetrics){
  ...
  return {ok:firstPayload.ok!==false,version:env.APP_VERSION||firstPayload.version||'15.0.6',workerVersion:env.APP_VERSION||'15.0.6',
...
async function databaseRankedForecast(env,normalized,c){
  ...
  const response=await fetch(`${base}/rest/v1/rpc/get_ranked_forecast`,{
    ...
    body:JSON.stringify({p_activity:normalized.activity,p_regions:normalized.regions,p_areas:normalized.areas,
      ...
      p_limit:FORECAST_ROWS_PER_DAY,p_version:env.APP_VERSION||'15.0.6'})
  });
  ...
  parsed.snapshotVersion=snapshotVersion;parsed.workerVersion=env.APP_VERSION||'15.0.6';parsed.version=env.APP_VERSION||'15.0.6';
...
async function buildCacheableForecast(env,normalized,c,totalStarted,canonicalUrl){
  ...
  result.headers={...result.headers,etag:resultEtag(snapshotVersion,canonicalUrl),'X-Vaderkompassen-Snapshot-Version':snapshotVersion,'X-Vaderkompassen-Worker-Version':env.APP_VERSION||'15.0.6','X-Vaderkompassen-Cached-At':new Date().toISOString()};
  return result;
}
...
async function status(env){
  ...
  return {ok:true,service:'Väderkompassen API',version:env.APP_VERSION||'15.0.6',workerVersion:env.APP_VERSION||'15.0.6',time:new Date().toISOString(),
...
async function appleSubscriptionOwner(env,subscription){
  if(subscription.appAccountToken)return subscription.appAccountToken;
  const rows=await sb(env,`subscriptions?provider=eq.apple&provider_subscription_id=eq.${encodeURIComponent(subscription.providerSubscriptionId)}&select=user_id&limit=1`);
  return rows?.[0]?.user_id||null;
}
async function saveVerifiedAppleSubscription(env,subscription,userId){
  const row=appleSubscriptionRow(subscription,userId);
  return sb(env,"rpc/sync_apple_subscription",{method:"POST",body:JSON.stringify(row)});
}
async function currentEntitlement(env,token){
  const result=await sb(env,"rpc/get_user_entitlement",{method:"POST",headers:{authorization:`Bearer ${token}`},body:"{}"});
  return Array.isArray(result)?result[0]:result;
}
async function appleClientSync(req,env,c){
  if(!appleConfigurationReady(env))throw Object.assign(new Error("Apple-verifieringen är inte konfigurerad"),{status:503});
  const {user,token}=await authenticatedUser(req,env),body=await req.json();
  const signedTransactions=Array.isArray(body?.signedTransactions)?body.signedTransactions:[body?.signedTransactionInfo].filter(Boolean);
  if(!signedTransactions.length||signedTransactions.length>20)throw Object.assign(new Error("En till tjugo Apple-transaktioner krävs"),{status:400});
  const synced=[];
  for(const signed of signedTransactions){
    const hint=decodeTransactionHint(signed),transactionId=hint.originalTransactionId||hint.transactionId;
    if(!transactionId)throw Object.assign(new Error("Apple-transaktionen saknar transactionId"),{status:400});
    const verified=await fetchVerifiedAppleSubscription(env,transactionId),owner=await appleSubscriptionOwner(env,verified);
    if(owner&&owner!==user.id)throw Object.assign(new Error("Apple-prenumerationen är kopplad till ett annat konto"),{status:409});
    if(!owner)throw Object.assign(new Error("Apple-transaktionen saknar ett giltigt appAccountToken"),{status:422});
    await saveVerifiedAppleSubscription(env,verified,user.id);synced.push(verified.providerTransactionId);
  }
  return json({ok:true,provider:"apple",synced:synced.length,entitlement:await currentEntitlement(env,token)},200,c);
}
async function sha256Hex(value){const bytes=new TextEncoder().encode(String(value));return [...new Uint8Array(await crypto.subtle.digest("SHA-256",bytes))].map(x=>x.toString(16).padStart(2,"0")).join("")}
async function appleServerNotification(req,env,c){
  if(!appleConfigurationReady(env))throw Object.assign(new Error("Apple-verifieringen är inte konfigurerad"),{status:503});
  const body=await req.json(),signedPayload=String(body?.signedPayload||"");
  if(!signedPayload||signedPayload.length>200000)throw Object.assign(new Error("signedPayload krävs"),{status:400});
  const {notification,subscription}=await verifyAppleNotification(env,signedPayload);
  if(subscription){
    const owner=await appleSubscriptionOwner(env,subscription);
    if(!owner)throw Object.assign(new Error("Verifierad Apple-prenumeration saknar kontokoppling"),{status:422});
    await saveVerifiedAppleSubscription(env,subscription,owner);
  }
  await sb(env,"apple_notification_events",{method:"POST",headers:{Prefer:"resolution=ignore-duplicates,return=minimal"},body:JSON.stringify({
    notification_uuid:notification.notificationUUID,notification_type:notification.notificationType||"UNKNOWN",subtype:notification.subtype||null,
    environment:String(notification.data?.environment||(configuredAppleEnvironment(env)==="Production"?"production":"sandbox")).toLowerCase(),original_transaction_id:subscription?.providerSubscriptionId||null,
    signed_at:notification.signedDate?new Date(notification.signedDate).toISOString():null,payload_hash:await sha256Hex(signedPayload)
  })});
...
export default {
  async fetch(req,env,ctx){
    ...
    try{
      if(url.pathname==='/'||url.pathname==='/health')return json({ok:true,service:'Väderkompassen API',version:env.APP_VERSION||'15.0.6',workerVersion:env.APP_VERSION||'15.0.6',time:new Date().toISOString()},200,c);
