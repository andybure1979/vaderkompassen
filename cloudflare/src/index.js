...
async function buildSnapshot(env){
  ...
  return {ok:true,version:'13.7.0',generatedAt:new Date().toISOString(),activeDate:days[0]||null,dailyResults,
...
async function latestSnapshot(env,url){
  ...
  return {ok:payload.ok!==false,version:payload.version||'13.7.0',generatedAt:stored.generated_at||payload.generatedAt,
...
async function status(env){
  ...
  return {ok:true,service:'Väderkompassen API',version:env.APP_VERSION||'13.7.0',time:new Date().toISOString(),
...
export default {
  async fetch(req,env){
    ...
    try{
      if(url.pathname==='/'||url.pathname==='/health')return json({ok:true,service:'Väderkompassen API',version:env.APP_VERSION||'13.7.0',time:new Date().toISOString()},200,c);
