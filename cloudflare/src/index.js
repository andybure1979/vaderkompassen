...
async function buildSnapshot(env){
  ...
  return {ok:true,version:'13.10.0',generatedAt:new Date().toISOString(),activeDate:days[0]||null,dailyResults,
...
async function latestSnapshot(env,url){
  ...
  // Bakåtkompatibel reserv innan första 13.10.0-snapshoten har skapats.
  ...
  return {ok:firstPayload.ok!==false,version:firstPayload.version||'13.10.0',generatedAt,
...
async function status(env){
  ...
  return {ok:true,service:'Väderkompassen API',version:env.APP_VERSION||'13.10.0',time:new Date().toISOString(),
...
export default {
  async fetch(req,env){
    ...
    try{
      if(url.pathname==='/'||url.pathname==='/health')return json({ok:true,service:'Väderkompassen API',version:env.APP_VERSION||'13.10.0',time:new Date().toISOString()},200,c);
