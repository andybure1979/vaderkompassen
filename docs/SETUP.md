...
5. Kör `npm run deploy:production`. Kommandot deployar root-entrypointen och stoppar om publik Worker-version inte är `14.4.3`.
6. Kontrollera att webbplatsen visar `Väderkompassen v14.4.3` i sidfoten.

Ingen databasändring krävs för v14.4.3. Efter Worker-deploy måste ett nytt snapshot byggas och visa `placesRequested=1000` samt `placesAvailable=1000` innan releasen betraktas som verifierad. Verifiera ett Free-anrop utan bearer-token och ett `access=premium`-anrop med Trial/Premium/VIP/Admin-session. Kontrollera att cacheheadern skiljer representationerna och att Free endast får första dagen.
