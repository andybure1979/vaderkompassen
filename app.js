...
const FORECAST_VALIDATORS_KEY="vk-forecast-validators-v15.0.1";
...
const diagnostics={version:"15.0.1",workerVersion:null,snapshotVersion:null,cache:null,etag:null,rowsRead:0,rowsReturned:0,responseBytes:0,totalMs:0,workerCpuApproxMs:0,supabaseCalls:0,mode:"checking",lastLoad:null,sources:[],forecastRequests:[]};
...
if(!window.VK_NATIVE?.isNativePlatform?.()&&"serviceWorker"in navigator)window.addEventListener("load",async()=>{
  ...
  const reg=await navigator.serviceWorker.register(`sw.js?v=15.0.1`);
